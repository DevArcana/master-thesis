...
Each rule might take as input an arbitrary set of working memory elements (WME), check arbitrary preconditions and produce arbitrary operators that when evaluated will change the state of the simulated world.
...
Based on the result of the action's execution the agent may store the estimated utility of the action in the memory and use it next time inside of the operator to determine if this action is worth using again.
This approach could be used in general to facilitate long-term agent's improvement as rules could be written to outright reject actions that bring no results before they are proposed.

\section{Simulation constraints and limitations}
